...
/**
 * 对 [okhttp3] 的请求和响应信息进行更规范和清晰的打印, 此类为框架默认实现, 以默认格式打印信息, 若觉得默认打印格式
...
class DefaultFormatPrinter : FormatPrinter {
     ...
     */
    override fun printJsonResponse(chainMs: Long, isSuccessful: Boolean, code: Int, headers: String,
                                   contentType: MediaType?, bodyString: String?,
                                   segments: List<String>, message: String,
                                   responseUrl: String) {
        var string = bodyString ?: ""
        string = when {
            RequestInterceptor.isJson(contentType) -> CharacterHandler.jsonFormat(string)
            RequestInterceptor.isXml(contentType) -> CharacterHandler.xmlFormat(string)
            else -> string
        }

        val responseBody = LINE_SEPARATOR + BODY_TAG + LINE_SEPARATOR + string
    ...
    companion object {
        ...
        private fun getTag(isRequest: Boolean): String {
            return TAG + if (isRequest) "-Request" else "-Response"
        }
    }
}
