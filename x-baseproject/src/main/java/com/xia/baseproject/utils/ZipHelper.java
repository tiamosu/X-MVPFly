...
public final class ZipHelper {
    ...
    public static String decompressToStringForZlib(byte[] bytesToDecompress, String charsetName) {
        final byte[] bytesDecompressed = decompressForZlib(bytesToDecompress);
    ...
    public static byte[] decompressForZlib(byte[] bytesToDecompress) {
        ...
        final int bufferSizeInBytes = numberOfBytesToDecompress;
        int numberOfBytesDecompressedSoFar = 0;
        final List<Byte> bytesDecompressedSoFar = new ArrayList<>();

        try {
            while (!inflater.needsInput()) {
                final byte[] bytesDecompressedBuffer = new byte[bufferSizeInBytes];
                final int numberOfBytesDecompressedThisTime = inflater.inflate(bytesDecompressedBuffer);
